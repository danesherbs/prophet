...
for (let i = 0; i < 12 * 10; i++) {
  ...
  console.log(
    "Salary:",
    state.getSalary().getYearlyGrossSalary(state.getClock().getTime())
  );
  console.log(
    "Bank balance:",
    state.getBank().getBalance(state.getClock().getTime())
  );
  console.log(
    "Super balance:",
    state.getSuper().getBalance(state.getClock().getTime())
  );
  console.log(
    "Stock balance:",
    state
      ...
      .reduce(
        (acc, stock) =>
          acc +
          stock.getNumberOfUnits() * stock.getPrice(state.getClock().getTime()),
        0
      )
  );

...
test('buy units buys correct number', () => {
    ...
    expect(() => { stock.buyUnits(0.1, 1) }).toThrow(TypeError);
    expect(() => { stock.buyUnits(1, 1.2) }).toThrow(TypeError);
});
...
test('sell units sells correct number', () => {
    ...
    expect(() => { stock.sellUnits(0, 0) }).toThrow(RangeError);
    expect(() => { stock.sellUnits(0, -10) }).toThrow(RangeError);
    expect(() => { stock.sellUnits(1, 14) }).toThrow(RangeError);
    expect(() => { stock.sellUnits(0.1, 1) }).toThrow(TypeError);
    expect(() => { stock.sellUnits(1, 1.2) }).toThrow(TypeError);
});

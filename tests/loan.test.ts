...
test('correct monthly repayments on interest only loan', () => {
    expect(interestOnlyLoan.getMonthlyInterestPayment())
        .toBeCloseTo(0, 10);
});

test('correct monthly repayments on principal and interest loan', () => {
    expect(principalAndInterestLoan.getMonthlyInterestPayment())
        .toBeCloseTo(28391, 10);
});

test('correct monthly interest rates', () => {
    expect(interestOnlyLoan.getMonthlyInterestRate())
        .toBeCloseTo(0.05, 10);

    expect(principalAndInterestLoan.getMonthlyInterestRate())
        .toBeCloseTo(0, 10);
});

...
test("correct yearly income tax with multiple brackets", () => {
  ...
  expect(tax.getYearlyIncomeTax(120_000)).toEqual(
    (40_000 - 1) * 0.2 + (60_000 - 1) * 0.5
  );
});
...
test("correct monthly income tax with multiple brackets", () => {
  ...
  expect(tax.getMonthlyIncomeTax(120_000)).toEqual(
    ((40_000 - 1) * 0.2 + (60_000 - 1) * 0.5) / 12
  );
});
...
test("correct net tax owed over last year", () => {
  const tax = new Tax({
    incomeTaxBrackets: new Array([[0.0, 120_000], 0.5]),
    superTaxRate: 0.15,
    declared: new Array(),
    paid: new Array(),
  })
  ...
  expect(tax.getNetUnpaidTaxOverLastTwelveMonths(0)).toEqual(0);

  expect(tax.getNetUnpaidTaxOverLastTwelveMonths(11)).toEqual(
    120_000 * 0.5 - 30_000 - 10_000
  );

  expect(tax.getNetUnpaidTaxOverLastTwelveMonths(13)).toEqual(0);
});
